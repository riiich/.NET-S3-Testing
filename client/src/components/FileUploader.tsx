...
import { createUser, getUserFiles, loginUser, uploadFile, deleteFile } from "../lib/api";
...
export default function FileUploader(): React.JSX.Element {
	...
	const deleteCurrFile = async (s3ItemId: number, userId: number): Promise<void> => {
		if (!currentUser) {
			setStatus("Sign in before deleting.");
			return;
		}
		
		try {
			await deleteFile(s3ItemId, userId);
			setStatus("File deleted.");
			await loadFiles(userId);
		} catch(error) {
			setStatus(error instanceof Error ? error.message : "There was an error deleting the file...");
		}
	};
							...
							<ul className="file-list">
								{files.map((file: S3Item) => (
									<li key={file.id}>
										...
										<button onClick={() => deleteCurrFile(file.id, file.userId)}>Delete</button>
									</li>
								))}
							</ul>
						)}
